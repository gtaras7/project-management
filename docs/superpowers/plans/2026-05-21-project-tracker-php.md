# DevBoard Project Tracker — PHP Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a project/task management app with plain HTML + Tailwind CDN + vanilla JS frontend backed by a PHP/MySQL REST API on MAMP.

**Architecture:** Single HTML page; vanilla JS manages state as plain objects; all data persisted in MySQL via `api.php`; `fetch()` handles every CRUD call; Tailwind CDN provides all styling — zero build step.

**Tech Stack:** HTML5, Tailwind CSS CDN, Vanilla JS (ES6+), PHP 8+, MySQL (PDO), MAMP
...
| File | Responsibility |
|------|----------------|
| `schema.sql` | DDL (CREATE TABLE) + seed data |
| `db_connect.php` | PDO singleton connection class |
| `api.php` | REST endpoint, action-routed via `?action=` |
| `index.html` | Page shell: Tailwind CDN, header, tab nav, modal HTML, `<script src="app.js">` |
| `app.js` | State object, API wrappers, all render/DOM functions, event handlers |

All five files go in the same folder, e.g. `/Applications/MAMP/htdocs/project-tracker/`.
...
1. Start MAMP → Apache + MySQL on
2. Open `http://localhost:8888/phpmyadmin/` (adjust port if you changed it from default)
3. Click **SQL** tab, paste the entire script, click **Go**
4. Expected: database `project_tracker` created; `projects` has 2 rows, `tasks` has 5 rows
...
- [ ] **Step 3: Copy files to MAMP htdocs**

```bash
mkdir -p /Applications/MAMP/htdocs/project-tracker
cp db_connect.php api.php schema.sql /Applications/MAMP/htdocs/project-tracker/
```

- [ ] **Step 4: Verify API in browser**

Open: `http://localhost:8888/project-tracker/api.php?action=projects`

Expected:
```json
[{"id":"proj-1","name":"PHP Backend Portfolio","description":"...","created_at":"..."},
 {"id":"proj-2","name":"E-Commerce Database Schema","description":"...","created_at":"..."}]
```

Open: `http://localhost:8888/project-tracker/api.php?action=tasks`
Expected: array of 5 task objects, each with `project_id`, `status`, `priority`, `deadline` fields.
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-900 pb-12 antialiased">

  <header id="app-header" class="bg-slate-900 text-white shadow-xl py-5 px-6 shrink-0 select-none border-b border-indigo-500/10"></header>

  <main class="max-w-7xl mx-auto px-4 sm:px-6 mt-8 space-y-6">
    <div id="tab-nav" class="flex overflow-x-auto pb-1 gap-2.5 border-b border-slate-200"></div>
    <div id="tab-content" class="space-y-6"></div>
  </main>

  <!-- Modal: Create Project -->
  <div id="modal-project" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full border border-slate-100 p-6 space-y-4">
      <h3 class="text-base font-bold text-slate-900 border-b border-slate-100 pb-3">Create New Course Project</h3>
      <form id="form-project" class="space-y-4">
        <div class="space-y-1.5">
          <label class="text-xs font-bold text-slate-700 block">Project Title</label>
          <input id="proj-name" type="text" placeholder="e.g. php-contact-form" required
            class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
          <p id="proj-name-error" class="text-xs text-rose-600 hidden">Project title is required.</p>
        </div>
        <div class="space-y-1.5">
          <label class="text-xs font-bold text-slate-700 block">Project Goal Description</label>
          <textarea id="proj-desc" rows="3" placeholder="Review relational dependencies, custom parameter bindings..."
            class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 resize-none"></textarea>
        </div>
        <div class="flex gap-3 justify-end pt-2 border-t border-slate-100">
          <button type="button" id="btn-cancel-project"
            class="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg">Cancel</button>
          <button type="submit"
            class="px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow">Save Project</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <!-- Modal: Create Task -->
  <div id="modal-task" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full border border-slate-100 p-6 space-y-4">
      <h3 class="text-base font-bold text-slate-900 border-b border-slate-100 pb-3">Record New Tracked Task</h3>
      <form id="form-task" class="space-y-4">
        <div class="space-y-1.5">
          <label class="text-xs font-bold text-slate-700 block">Task Name</label>
          <input id="task-title" type="text" placeholder="e.g. Code password validation middleware" required
            class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
          <p id="task-title-error" class="text-xs text-rose-600 hidden">Task name is required.</p>
        </div>
        <div class="space-y-1.5">
          <label class="text-xs font-bold text-slate-700 block">Description Details</label>
          <textarea id="task-desc" rows="3"
            class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 resize-none"></textarea>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-700 block">Associated Project</label>
            <select id="task-project" required
              class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"></select>
          </div>
          <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-700 block">Target Deadline</label>
            <input id="task-deadline" type="date"
              class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
          </div>
          <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-700 block">Priority Level</label>
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold text-slate-700 block">Starting Status</label>
        ...
        <div class="flex gap-3 justify-end pt-2 border-t border-slate-100">
          <button type="button" id="btn-cancel-task"
            class="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg">Cancel</button>
          <button type="submit"
            class="px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow">Save Task Block</button>
        </div>
      </form>
    </div>
  </div>
...
- [ ] **Step 2: Copy to htdocs and verify shell loads**

```bash
cp index.html /Applications/MAMP/htdocs/project-tracker/
```

Open `http://localhost:8888/project-tracker/index.html` — page background should be off-white (slate-50), no console errors.

---

### Task 4: app.js — State, API Layer, Init

**Files:**
- Create: `app.js`

- [ ] **Step 1: Write app.js — state + API wrappers + init**
...
```bash
cp app.js /Applications/MAMP/htdocs/project-tracker/
```

- [ ] **Step 3: Full end-to-end verification**

Open `http://localhost:8888/project-tracker/index.html`. Verify:

1. **Header** — dark bar with title, PHP version badge, Projects/Completed/Overdue stats pills
2. **Kanban board** — 4 columns each showing correct task cards; priority badges, deadline alerts correct
3. **Project filter tabs** — clicking "PHP Backend Portfolio" shows only its 3 tasks; "All Projects" shows all 5
4. **Search** — type "PDO" → only matching cards visible
5. **Priority filter** — select "High" → only high-priority cards visible; clear → all return
6. **Move task** — click → on a card in "To Do"; card moves to "In Progress"; header stats update; refresh page → change persisted in DB
7. **Delete task** — click 🗑, confirm → card removed; refresh → still gone from DB
8. **New Project modal** — click "+ New Project", submit empty → error shown; fill name, submit → new project tab appears, header Projects count increments
9. **Add Task modal** — click "+ Add Task", submit empty → error shown; fill form, submit → task appears in correct column; refresh → persisted
10. **Deadlines tab** — click "Urgent Deadlines Tracker"; tasks sorted into correct categories; rows show deadline date chip, status badge
```
